...
const PricingModal: React.FC<PricingModalProps> = ({ user, onClose, onUpgrade }) => {
  ...
  useEffect(() => {
    const fetchSpots = async () => {
      try {
        const q = query(
          collection(db, 'users'),
          where('subscriptionPlan', 'in', ['pro', 'business'])
        );
        const snapshot = await getCountFromServer(q);
        const count = snapshot.data().count;
        setSpotsLeft(Math.max(0, 1000 - count));
      } catch (err) {
        setSpotsLeft(1000); // Fallback if query fails
      }
    };
    fetchSpots();
  }, []);
  ...
  const getAmount = (plan: 'pro' | 'business') => {
    if (plan === 'pro') {
      return (billing === 'monthly' ? 149 : 799) * 100;
    }
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-in fade-in duration-300">
      <div className="relative w-full max-w-6xl bg-[#050510] rounded-[2.5rem] border border-white/10 shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        >
          ...
          <>
            {/* Header Section */}
            <div className="text-center pt-8 pb-6 px-6 bg-gradient-to-b from-purple-900/20 to-transparent">
              <div className="inline-flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/20 px-4 py-2 rounded-full mb-6">
                ...
                <span className="text-xs font-bold text-emerald-400 uppercase tracking-widest">
                  Early Adopter Beta: {spotsLeft !== null ? `${spotsLeft} Spots Left` : 'Checking spots...'}
                </span>
              </div>
              ...
              <p className="text-gray-400 text-sm max-w-xl mx-auto mb-4">
                We're currently in closed beta finalizing our scaling infrastructure. Grab a spot before prices increase to their standard retail value.
              </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
                ...
                <div className="rounded-3xl border border-white/10 bg-white/5 p-8 flex flex-col hover:border-white/20 transition-all">
                  <div className="mb-6">
                    ...
                    <h3 className="text-xl font-black text-white">Cadet</h3>
                  </div>
                  <div className="mb-6">
                ...
                <div className="rounded-3xl border-2 border-purple-500 bg-[#0f0f29] p-8 flex flex-col relative transform md:-translate-y-4 shadow-2xl shadow-purple-500/20">
                  ...
                  <div className="mb-6">
                    ...
                    <h3 className="text-xl font-black text-white">Orbital</h3>
                  </div>
                  
                  <div className="mb-6">
                    <div className="flex flex-col mb-1">
                      <span className="line-through text-gray-500 font-bold text-lg decoration-red-500/50">
                        {billing === 'monthly' ? 'R499' : 'R4,999'}
                      </span>
                    </div>
                    <div className="flex items-end gap-1">
                      <p className="text-5xl font-black text-white tracking-tight">
                        {billing === 'monthly' ? 'R149' : 'R799'}
                      </p>
                    ...
                    {billing === 'yearly' && (
                      <div className="mt-2 text-emerald-400 text-xs font-black uppercase tracking-wider flex items-center gap-1">
                        <ShieldCheck size={12} /> Save up to R4,200 vs retail
                      </div>
                    )}
                  </div>
                  ...
                  >
                    {isProcessing && selectedPlan === 'pro' ? <Loader2 className="animate-spin" size={20} /> : 'Get Orbital'}
                  </button>
                </div>

                {/* Business Tier (Constellation) */}
                <div className="rounded-3xl border border-white/10 bg-white/5 p-8 flex flex-col hover:border-amber-500/50 hover:bg-amber-500/5 transition-all group">
                  <div className="mb-6">
                    <h3 className="text-xl font-black text-white group-hover:text-amber-400 transition-colors">Constellation</h3>
                  </div>
                  
                  <div className="mb-6">
                    <div className="flex flex-col mb-1">
                      <span className="line-through text-gray-500 font-bold text-lg decoration-red-500/50">
                        {billing === 'monthly' ? 'R1,499' : 'R14,999'}
                      </span>
                    </div>
                    <div className="flex items-end gap-1">
                  ...
                  <div className="space-y-3 mb-8 flex-1">
                    {['Everything in Orbital', 'AI Business Plan & Logo Gen', 'Unlimited Funding Scans', 'Financial Projections', 'Priority 24/7 Support'].map((feat, i) => (
                      <div key={i} className="flex items-center gap-3">
                  ...
                  >
                    {isProcessing && selectedPlan === 'business' ? <Loader2 className="animate-spin" size={20} /> : 'Join Constellation'}
                  </button>
                </div>
